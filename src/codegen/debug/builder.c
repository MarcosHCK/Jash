...
{
#if OUTPUT_ON_FILE
  static guint filecount = 0;
  gchar* filename = g_strdup_printf ("closure.%i", filecount++);
  builder_abfd (builder) = bfd_openw (filename, "default");
                          g_free (filename);
#else // OUTPUT_ON_FILE
  ...
  const flagword file_flags = D_PAGED;
  ...
  if (!bfd_set_start_address (abfd, 0))
    g_error ("(" G_STRLOC "): bfd_set_start_address ()!: %s", bfd_lasterr ());
...
{
  asymbol* sym = (gpointer) j_gdb_builder_decl_symbol (builder, name, address, section);
return (sym->flags |= BSF_FUNCTION, (JGdbSymbol*) sym);
}
...
{
  asymbol* sym = (gpointer) j_gdb_builder_decl_symbol (builder, name, address, section);
return (sym->flags |= BSF_OBJECT, (JGdbSymbol*) sym);
}

JGdbSymbol* j_gdb_builder_decl_section_symbol (JGdbBuilder* builder, const gchar* name, gpointer address, JGdbSection* section)
{
  asymbol* sym = (gpointer) j_gdb_builder_decl_symbol (builder, name, address, section);
return (sym->flags |= BSF_SECTION_SYM, (JGdbSymbol*) sym);
}

JGdbSymbol* j_gdb_builder_decl_symbol (JGdbBuilder* builder, const gchar* name, gpointer address, JGdbSection* section)
{
  bfd* abfd = builder_abfd (builder);
...
JGdb* j_gdb_builder_end (JGdbBuilder* builder)
{
  ...
  if (!bfd_set_symtab (abfd, (asymbol**) symbols->pdata, symbols->len - 1))
